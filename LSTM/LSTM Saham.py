...
dftlkm = pd.read_csv('C:/Users/Jarpul/Downloads/TLKM.JK.csv',)

column_list=['Date','Open']
df_final=dftlkm[column_list]                                                                         
df_final.head(3)
...
plt.xticks(range(0,dftlkm.shape[0],80),dftlkm['Date'].loc[::80])
...
np.savetxt("ScaledDataTLKM.csv", file, '%s', delimiter=",")
...
np.savetxt("PredictionsTLKM.csv", file, '%s', delimiter=",")

mape = np.mean(np.abs(predictions - yTest)/np.abs(yTest))
print('MAPE: '+str(mape))

# Add prediction for Plot
train = dftlkm.loc[:trainingDataLen, ['Date', 'Open'] ]
valid = dftlkm.loc[trainingDataLen:, ['Date', 'Open'] ]
...
plt.title('Grafik Hasil Prediksi')
plt.xlabel('Date', fontsize=20)
plt.xticks(range(0,dftlkm.shape[0],25),dftlkm['Date'].loc[::2])
plt.ylabel('Harga Penutupan (IDR)', fontsize=20)
plt.plot(validation[['Open', 'Predictions']])
plt.legend(['Actual', 'Predictions'], loc='lower right')
plt.show()

# Validating predicted price stock
validation

file = np.asarray(validation)
np.savetxt("ValidationTLKM.csv", file, '%s', delimiter=",")
